...
> [!IMPORTANT]
> These notes are prepared for the `v2.5.0-rc2` release candidate. RC2
> supersedes RC1 for testing after correcting the fan-out retry lifecycle and
> adding structured delivery diagnostics. Availability and deployment remain
...
Before RC2 publication and production testing:

- merge the retry-lifecycle and delivery-diagnostics changes after green CI;
- build and validate the exact RC2 Debian and multi-architecture container
  artifacts;
- repeat the native-package upgrade, reinstall, rollback, identity, Redis, and
  removal gates against RC2; and
- publish and independently verify RC2 before a controlled production upgrade
  and high-fan-out soak test.
